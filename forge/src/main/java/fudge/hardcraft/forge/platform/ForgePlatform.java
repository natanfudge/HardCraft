package fudge.hardcraft.forge.platform;

import fudge.hardcraft.HardCraft;
import fudge.hardcraft.platform.CommonModMetadata;
import fudge.hardcraft.platform.ModsByLocation;
...
public class ForgePlatform implements NecPlatform {
    ...
    public boolean modContainsFile(CommonModMetadata mod, String path) {
        ...
        try (FileSystem fs = FileSystems.newFileSystem(mod.rootPath())) {
            ...
            HardCraft.getLogger().error("Failed to open mod jar, assuming it doesn't contain file " + path, e);
            return false;
        }
    }
